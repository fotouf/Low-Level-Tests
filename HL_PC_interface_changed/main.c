...
//	Start_Continious_Mode();
	...
	DMA_SetCurrDataCounter(DMA2_Stream1, MESS_LENGTH_RX);
...
static void USART_Config(void)
{
  ...
  */
  ...
  /* Configure RX DMA */
  DMA_InitStructure.DMA_BufferSize = MESS_LENGTH_RX ;
  DMA_InitStructure.DMA_Channel = DMA_Channel_5 ;
...
//
//	counter = counter + 1;
//	if (counter>16000){led_on(yellow);}
//
//
...
//	//Enable again the stream and the request.        //
//
//	DMA_SetCurrDataCounter(DMA2_Stream1, MESS_LENGTH_RX);
//
//	DMA_Cmd(DMA2_Stream1,ENABLE);
...
{
	DMA_ClearFlag(DMA2_Stream1,DMA_FLAG_TCIF1);					//NOTE: When transmission complete, the DMA stream is disabled! No DMA request is served unless the software re-programs the stream and re-enables it!!

	//USART_DMACmd(USART6, USART_DMAReq_Rx, DISABLE);

//	if(counter>100){led_off(green1);}
//	if(counter>200){led_on(green1);}
...
//
		    	             	//led_on(green2);
		    	             	 counter=counter+1;
		    	             	if(counter>1000){led_off(green1);}
		    	             	//if(counter>2000){led_on(green1);}
...
//	led_on(yellow);
//	DMA_Cmd(DMA2_Stream1,ENABLE);
//	while(DMA_GetCmdStatus(DMA2_Stream1) != ENABLE);			//Maybe we need a timeout here
//
//	led_off(yellow);
