...
//  STM_EVAL_PBInit(BUTTON_TAMPER,BUTTON_MODE_GPIO);
  ...
  if(DMA_GetCurrDataCounter(USARTx_TX_DMA_STREAM)==BUFFERSIZE){led_on(green2);}
...
// 	 while (STM_EVAL_PBGetState(BUTTON_TAMPER));
  ...
  while (DMA_GetFlagStatus(USARTx_TX_DMA_STREAM,USARTx_TX_DMA_FLAG_TCIF)==RESET){toggle_led(red);}
  led_on(yellow);
  if(DMA_GetCurrDataCounter(USARTx_TX_DMA_STREAM)==0){led_off(green2);}
